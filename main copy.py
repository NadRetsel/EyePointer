...
def main():
    ...
    while True:
        ...
        annotated_image, results = getFaceMesh(image)

        if results.multi_face_landmarks:
            calculateFaceAngle(results, annotated_image)


        cTime = time.time()
        fps = 1 / (cTime - pTime)
        cv2.putText(annotated_image, str(int(fps)), (10,400), cv2.FONT_HERSHEY_PLAIN, 3, (255,0,0), 3)
